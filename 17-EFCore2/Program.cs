...
namespace BooksSample
{
    class Program
    {
        ...
        private static (BooksContext context, Book book) PrepareUpdate()
        {
            ...
            if(book == null){
                Console.WriteLine($"could not found book with title :{BookTitle}");
            }
            Console.Write($"updating book :[{book.Title}, {book.BookId}]");
            return (context, book);
        }
        ...
        private async Task Add4BooksAsync()
        {
            Console.WriteLine(nameof(Add4BooksAsync));
            using (var context = new BooksContext())
            {
                ...
                await context.BookAuthors.AddRangeAsync(ba1, ba2, ba3, ba4);
        ...
        private async Task UpdateBookAsync(string title = "Professional C# 7")
        {
            using (var context = new BooksContext())
            {
                int records = 0;
                ...
                if (book != null)
                {
                    book.Title = "Professional C# 7 and .NET Core 2.0[updated]";
        ...
        private async Task DeleteAllBookAsync()
        {
            using (var context = new BooksContext())
            {
